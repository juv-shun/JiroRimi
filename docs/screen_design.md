...
**カテゴリカラー**:
| カテゴリ | 条件 | バッジ背景 | バッジテキスト |
|----------|------|------------|----------------|
| じろカップ（Boys） | is_boys=true, is_girls=false | `#DBEAFE`（ライトブルー） | `#1E40AF`（ブルー） |
| りみカップ（Girls） | is_boys=false, is_girls=true | `#FCE7F3`（ライトピンク） | `#BE185D`（ピンク） |
| Jiro-Rimi Cup（Both） | is_boys=true, is_girls=true | `#FFF3E6`（ライトオレンジ） | `#FF7A00`（オレンジ） |
...
| 7 | SCR-002-F07 | マイエントリー一覧 | 自分がエントリーした予選の一覧を表示 | 1.5.8 |
...
| 2 | SCR-003-F02 | カテゴリフィルタリング | ログインユーザーの性別に合った大会のみ表示（is_boys/is_girls で判定） | 1.5.1 |
...
| カテゴリ | 「じろカップ」/「りみカップ」/「Jiro-Rimi Cup」バッジ（is_boys/is_girls の組み合わせで決定） |
| ステータス | 公開中 / 進行中 等 |
| 予選回数 | 全予選数 |
...
**表示条件**:
- 未ログイン: すべての公開中大会を表示
- ログイン済み（boys）: `is_boys = true` の大会のみ表示
- ログイン済み（girls）: `is_girls = true` の大会のみ表示
...
| 6 | SCR-004-F06 | カテゴリチェック | ユーザーの性別に対応する is_boys/is_girls フラグを照合 | 1.5.6 |
...
| カテゴリ不一致（ユーザーが boys なのに is_boys=false 等） | 非表示または「参加資格なし」 |
...
| No | 機能ID | 機能名 | 説明 | 対応バックログ |
|----|--------|--------|------|---------------|
| 1 | SCR-007-F01 | 大会基本情報入力 | 大会名・カテゴリ（is_boys/is_girls）を入力 | 1.4.2 |
...
| じろカップ（Boys） | チェックボックス | ○（少なくとも1つ） | false | is_boys フラグ |
| りみカップ（Girls） | チェックボックス | ○（少なくとも1つ） | false | is_girls フラグ |
...
**バリデーション**:
- 大会名: 1-100文字
- カテゴリ: is_boys, is_girls のうち少なくとも1つは true
