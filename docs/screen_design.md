...
| 区分 | バッジ背景 | バッジテキスト |
|------|------------|----------------|
| じろカップ（Boys） | `#DBEAFE`（ライトブルー） | `#1E40AF`（ブルー） |
| りみカップ（Girls） | `#FCE7F3`（ライトピンク） | `#BE185D`（ピンク） |
...
| 10 | SCR-002-F10 | 初回登録モード | 初回ログイン時は必須項目入力完了まで離脱不可 | 1.3.8 |
...
- ウェルカムメッセージを表示（「じろりみへようこそ！」等）
- 必須項目の入力進捗をステップ表示（例: 1/4, 2/4...）
- サイドメニューを非表示または非活性化
- ログアウトボタンのみ操作可能（他画面への遷移は不可）
- 全必須項目の入力完了後に「登録完了」ボタンで通常モードへ移行
- 登録完了後、大会一覧画面へ自動遷移
