...
| No | 項目 | 現在の整理 | 対応バックログ |
|----|------|-----------|---------------|
| 1 | 参加日数が異なる場合のランキング評価方法 | 総勝利数・勝率・強さ指標のどれを採用するかを決定する | 2.6.1, 2.6.4 |
| 2 | 奇数人数の場合のチーム分け | BYE・不戦勝・運営調整の扱いを決定する | 2.3.2 |
| 3 | 参加者の途中離脱時の扱い | 残り試合のチーム再編成ルールを決定する | 2.4.9 |
...
| 2.4.9 | 予選ステータス遷移 | チェックイン受付中→イベント開始（参加メンバー確定）→チーム編成→試合進行→予選終了の遷移を管理 | 各フェーズで適切なUIが表示される | ⚪ Backlog |
