...
| フェーズ | 目標 | ステータス |
|---------|------|-----------|
| **Phase 1** | 大会のエントリーができる | 🔵 Not Started |
...
|----|-------------------|------|-------------|-----------|
| 1.1.1 | Firebaseプロジェクト初期設定 | Firebase プロジェクトを作成し、Auth/Firestore/Hosting/Functionsを有効化 | 各サービスがコンソールで有効になっている | 🔵 Not Started |
| 1.1.2 | フロントエンド環境構築 | React + TypeScript + Vite でフロントエンドプロジェクトを初期化 | `pnpm dev` でローカル起動できる | 🔵 Not Started |
| 1.1.3 | バックエンド環境構築 | TypeScript + Firebase Cloud Functions の開発環境を構築 | ローカルエミュレータで関数が動作する | 🔵 Not Started |
| 1.1.4 | Firestore データモデル設計 | ユーザー/大会/予選/エントリーのスキーマを定義 | ドキュメント化されたスキーマが存在する | 🔵 Not Started |
| 1.1.5 | Firestore セキュリティルール | 認証・権限に基づくアクセス制御ルールを実装 | 未認証ユーザーは書き込み不可 | 🔵 Not Started |
...
| ID | バックログアイテム | 説明 | 受け入れ条件 | ステータス |
|----|-------------------|------|-------------|-----------|
| 1.2.1 | X（Twitter）OAuth 設定 | Firebase Auth に X プロバイダーを設定 | Twitter Developer Portal と連携完了 | 🔵 Not Started |
| 1.2.2 | ログイン画面 | X アカウントでログインできる UI を実装 | ログインボタン押下で X 認証フローが開始される | 🔵 Not Started |
...
| 1.2.5 | ユーザー情報の自動保存 | 初回ログイン時に X ID を Firestore に保存 | ログイン後、users コレクションにドキュメントが作成される | 🔵 Not Started |
...
| 1.3.2 | プレイヤー名設定 | ゲーム内プレイヤー名を入力・保存できる | 入力した名前が Firestore に保存される | 🔵 Not Started |
| 1.3.3 | Discord ID 設定 | Discord ID（必須）を入力・保存できる | バリデーション付きで保存される | 🔵 Not Started |
| 1.3.4 | 性別設定 | ボーイズ / ガールズ を選択できる | 選択した性別が保存される | 🔵 Not Started |
| 1.3.5 | ロール優先順位設定 | 5種のロールから3つを選び優先順位付け | 第1〜第3希望が保存される | 🔵 Not Started |
| 1.3.6 | X ID 表示（編集不可） | 認証から取得した X ID を表示 | 自動取得され編集不可で表示される | 🔵 Not Started |
...
| ID | バックログアイテム | 説明 | 受け入れ条件 | ステータス |
|----|-------------------|------|-------------|-----------|
| 1.4.1 | 運営者権限の付与機能 | 特定ユーザーに運営者権限を付与する仕組み | Firestore で運営者フラグを管理できる | 🔵 Not Started |
...
|------|------|------|-------------|-------------|
| 1.1 プロジェクト基盤構築 | 5 | 0 | 0 | 5 |
| 1.2 ユーザー認証 | 5 | 0 | 0 | 5 |
| 1.3 プロフィール設定 | 7 | 0 | 0 | 7 |
| 1.4 大会作成・管理 | 8 | 0 | 0 | 8 |
| 1.5 大会エントリー | 8 | 0 | 0 | 8 |
| 1.6 エントリー一覧・公開 | 4 | 0 | 0 | 4 |
| **Phase 1 合計** | **37** | **0** | **0** | **37** |
