...
| 1.4.3 | 大会作成画面 | 大会の基本情報と予選設定を含む作成フォームを実装 | 大会名・性別区分・予選日程・エントリー期限・チェックイン時間帯・試合数・エントリー方式・進行形式・GF進出人数を入力して作成できる | 🟢 Done |
...
| ID | バックログアイテム | 説明 | 受け入れ条件 | ステータス |
|----|-------------------|------|-------------|-----------|
| 2.2.0 | チェックイン用マイグレーション | entries テーブルに checked_in_at カラムを追加し、RLS ポリシーを拡張 | ・entries テーブルに checked_in_at (timestamptz, nullable) カラムが追加される<br>・参加者が自身のエントリーの checked_in_at を更新できる RLS ポリシーが設定される<br>・管理者が全エントリーの checked_in_at を更新できる RLS ポリシーが設定される<br>・マイグレーションがローカル・本番で動作する | ⚪ Backlog |
| 2.2.1 | 参加者チェックイン機能 | エントリー済み予選に対してチェックインできる | ・大会詳細画面でエントリー済み予選にチェックインボタンが表示される<br>・チェックイン可能時間帯のみボタンが活性化される<br>・チェックイン実行後、checked_in_at に現在時刻が保存される<br>・チェックイン済みの場合は「チェックイン済み」と表示される | ⚪ Backlog |
